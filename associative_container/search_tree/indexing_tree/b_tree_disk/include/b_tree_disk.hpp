...
void B_tree_disk<tkey, tvalue, compare, t>::rebalance_node(
    ...
    // 1: Попытаться заимствовать ключ у левого соседа
    ...
    // 2: Попытаться заимствовать ключ у правого соседа
    ...
    //3: Объединение с соседом

    // 3.1: Объединение с левым соседом
    if (node_idx_in_pointers > 0) {
        size_t left_sibling_pos = parent.pointers[node_idx_in_pointers - 1];
        btree_disk_node left_sibling = disk_read(left_sibling_pos);
    ...
    // 3.2: Объединение с правым соседом
    if (node_idx_in_pointers <= parent.size) {
        size_t right_sibling_pos = parent.pointers[node_idx_in_pointers + 1];
        btree_disk_node right_sibling = disk_read(right_sibling_pos);
...
B_tree_disk<tkey, tvalue, compare, t>::B_tree_disk(
    ...
    if (!files_exist) {
        ...
        if (_count_of_node == 0 || _position_root == static_cast<size_t>(-1)) {
            throw std::runtime_error("B-tree file is invalid or empty");
        }
