...
B_tree<tkey, tvalue, compare, t>::erase(const tkey& key) {
    ...
    auto borrow_from_prev = [&](btree_node* parent, size_t idx) {
        // child - текущий узел (которому не хватает ключей)
        // sibling - его левый сосед (у которого берём ключ)
        btree_node* child = parent->_pointers[idx];
        btree_node* sibling = parent->_pointers[idx - 1];

        // Ключ из родителя перемещается в ребёнка
        child->_keys.insert(child->_keys.begin(), parent->_keys[idx - 1]);

        //Если есть потомки, крайний правый потомок соседа переходит к ребёнку
    ...
    auto merge = [&](btree_node* parent, size_t idx) {
        // child - текущий узел (который будет "поглощать" соседа)
        // sibling - его правый сосед (который удаляется)
        btree_node* child = parent->_pointers[idx];
        btree_node* sibling = parent->_pointers[idx + 1];

        // Разделяющий ключ из родителя добавляется в ребёнка
        child->_keys.push_back(parent->_keys[idx]);

        // Все ключи соседа добавляются в ребёнка
        child->_keys.insert(child->_keys.end(), sibling->_keys.begin(), sibling->_keys.end());

        // Если есть потомки, они также переносятся
        if (!sibling->_pointers.empty()) {
            child->_pointers.insert(child->_pointers.end(), sibling->_pointers.begin(), sibling->_pointers.end());
        }

        // Удаляем разделяющий ключ из родителя
        parent->_keys.erase(parent->_keys.begin() + idx);

        // Удаляем указатель на соседа
        parent->_pointers.erase(parent->_pointers.begin() + idx + 1);

        // Освобождаем память соседа
        delete sibling;
    };
